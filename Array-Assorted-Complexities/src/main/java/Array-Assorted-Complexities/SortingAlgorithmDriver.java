...
public final class SortingAlgorithmDriver {
  ...
  private static List<SortingAlgorithm<String>> getSortingAlgorithms() {
    ...
    sorts.add(new hw3.sort.InsertionSort<>()); // TAs: use sln.InsertionSort.
    return sorts;
  }
  ...
  private static MeasuredIndexedList<String> toArray(List<String> list,
                                                     int toCopy) {
    MeasuredIndexedList<String> array = new MeasuredIndexedList<>(toCopy, null);
  ...
  private static void updateReport(String dataFile, String algo,
                                   MeasuredIndexedList<String> array,
                                   double time) {
    if (report == null) {
      initReport();
    }
  ...
  private static void runSortAlgoOnData(SortingAlgorithm<String> sortAlgo,
    ...
    MeasuredIndexedList<String> data = toArray(readData(dataFile), SIZE);
    data.reset();
